...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        if self.ffn_type == "ndlinear":
            self.fc1_ndlinear = NdLinear(config.ffn_ndlinear_in_dim, config.ffn_ndlinear_ndlinear_dim)
            self.fc2_ndlinear = NdLinear(config.ffn_ndlinear_ndlinear_dim, config.ffn_ndlinear_in_dim)
        else:
    ...
    def forward(
        ...
        if self.ffn_type == "ndlinear":
            ...
            hidden_states = self.fc1_ndlinear(hidden_states)
            hidden_states = self.activation_fn(hidden_states)
            hidden_states = self.fc2_ndlinear(hidden_states)
