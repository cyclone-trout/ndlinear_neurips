...
def setup_distributed():
    ...
    # Ensure you are on the right device when calling dist.barrier()
...
def main():
    ...
    # gpus_per_node = torch.cuda.device_count()    # gpus_per_node = int(os.environ["SLURM_GPUS_ON_NODE"])
    #
    # assert gpus_per_node == torch.cuda.device_count()
    # print(f"Hello from rank {local_rank} of {world_size} on {gethostname()} where there are" \
    ...
    # Manually Save model state_dict to a .bin file
    os.makedirs(args.output_dir, exist_ok=True)
    torch.save(model, f"{args.output_dir}/pytorch_model.bin")
    logger.info(f"Model structure saved to {args.output_dir}/pytorch_model.bin")
    ...
    # lr_scheduler from accelerator didn't work as expected!
    # model, optimizer, train_dataloader, eval_dataloader, lr_scheduler = accelerator.prepare(
    #     model, optimizer, train_dataloader, eval_dataloader, lr_scheduler
    # )
    model, optimizer, train_dataloader, eval_dataloader, _ = accelerator.prepare(
        model, optimizer, train_dataloader, eval_dataloader, lr_scheduler
    )
    ...
    print("DEBUG printing: ", args.num_train_epochs)
    # exit()
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                # ZZ - WandB
                # print(f"Step {completed_steps} loss: {loss}", flush=True)
                
                accelerator.backward(loss)
                
                # local_lr = torch.tensor(optimizer.param_groups[0]["lr"]).to(accelerator.device)
                # all_lrs = accelerator.gather(local_lr)
                # if accelerator.is_main_process:
                #     for i, lr in enumerate(all_lrs):
                #         print(f"GPU {i}: Learning Rate = {lr.item()}", flush=True)
